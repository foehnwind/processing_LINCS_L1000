function [ b,sigCount] = chdirFull( ctrl,expm,r )
...
n1 = size(ctrl,2);
n2 = size(expm,2);
ctrlIdx = false(size(combinedData,2),1);
ctrlIdx(1:n1) = true;
expmIdx = ~ctrlIdx;
...
componentsCount = min([samplesCount-1,30]);
...
cutIdx = find(cumsum(pcvars) > 0.999);
if isempty(cutIdx)
...
%% Calculate Sig Genes. <This part is definitely right FINAL!!!>

nnull = 10;
nu = n1+n2-2;
sqrtPart = sqrt(nu./chi2rnd(nu,1,nnull));
y1 = mvnrnd(zeros(nnull,cutIdx),Dd).*repmat(sqrtPart',1,cutIdx);
sqrtPart = sqrt(nu./chi2rnd(nu,1,nnull));
y2 = mvnrnd(zeros(nnull,cutIdx),Dd).*repmat(sqrtPart',1,cutIdx);
y = y2-y1;

bns = zeros(size(ctrl,1),nnull);
for i = 1:nnull
   bn = V/shrunkMats*V'*(y(i,:)*V')';
   bn = bn/norm(bn);
   bn = bn.^2;
   bns(:,i) = sort(bn,'descend');
end

chdirNull = mean(bns,2);
relerr = sort(b.^2,'descend')./chdirNull;
relerr = cumsum(relerr)/sum(relerr)-((1:numel(meanvec))/numel(meanvec))';
[~,sigCount] = max(relerr);
